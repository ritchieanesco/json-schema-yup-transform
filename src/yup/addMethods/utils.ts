...
import type { JSONSchema, JSONSchemaBasicType } from "../../schema";

/**
 * Checks if input is one of enum
 */
export const isValueEnum = (
  enums: JSONSchema["enum"],
  value: unknown
): boolean => isArray(enums) && enums.some((item) => isEqual(item, value));
